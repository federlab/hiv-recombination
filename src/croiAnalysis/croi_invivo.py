...
rcParams.update({'font.size': 18,'figure.figsize':(10, 9) })
linewidth = 1
...
#save a version without the Neher-Leitner estimate labeled
plt.savefig(outDir + 'ests_invivo_' + str(NUM_BOOTSTRAPS) + 'no_neher.jpg', dpi = 300)
...
plt.savefig(outDir + 'ests_invivo_' + str(NUM_BOOTSTRAPS) + '.jpg', dpi = 300)
plt.close()



####################### Plotting The Fit######################################
rcParams.update({'font.size': 18,'figure.figsize':(7, 5) })
...
sns.lineplot(x ='bin_edges', y ='ratio_bins', data = low_50k, color = 'tab:blue', linewidth = linewidth, label = 'Low VL')
sns.lineplot(x ='bin_edges', y ='mid_conf', data = low_50k, color = 'navy',linewidth = linewidth)
sns.lineplot(x ='bin_edges', y ='lower_conf', data = low_50k, color = 'navy', linestyle = 'dashed', linewidth = linewidth)
sns.lineplot(x ='bin_edges', y ='high_conf', data = low_50k, color = 'navy', linestyle = 'dashed', linewidth = linewidth)

sns.lineplot(x ='bin_edges', y ='ratio_bins', data = high_50k, color = 'tab:orange', linewidth = linewidth, label = 'High VL')
sns.lineplot(x ='bin_edges', y ='mid_conf', data = high_50k, color = 'saddlebrown', linewidth = linewidth)
sns.lineplot(x ='bin_edges', y ='lower_conf', data = high_50k, color = 'saddlebrown', linestyle = 'dashed', linewidth = linewidth)
sns.lineplot(x ='bin_edges', y ='high_conf', data = high_50k, color = 'saddlebrown', linestyle = 'dashed', linewidth = linewidth)

plt.xlabel(r'Distance $\cdot$ Time (bp $\cdot$ generation)')
plt.ylabel("D\' Ratio")
plt.tight_layout()
plt.savefig(outDir + 'fits_invivo_' + str(NUM_BOOTSTRAPS) + '.jpg', dpi = 300)
plt.close()