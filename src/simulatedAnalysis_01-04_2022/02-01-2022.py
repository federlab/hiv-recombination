...
#02-28-2022: Today I am adjusting the script to do better curve fits
...
for curr_dataset in os.listdir(dataDir + enclosing_dir):
    ...
    for i in range(len(bin_lists)):
        ...
        #get the intercept
        intercept_coeffs, intercept_fit = neher.run_neher_fit(c0_fixed = False, lower_bounds = [0,0,0],
                                    upper_bounds = [1,1,1], 
                                    initial = [0.13, 0.1804, 0.000114],
                                    test_results = all_frequencies)

        intercept = intercept_coeffs[0]

        #perform our fitting
        coeffs, fit_df = neher.run_neher_fit(c0_fixed = True, lower_bounds = [0,0],
                                    upper_bounds = [1,1], 
                                    initial = [intercept, 0.1804, 0.000114],
                                    test_results = all_frequencies)
        ...
        plt.tight_layout()
        plt.savefig(outDir + enclosing_dir + curr_dataset + "/neherResults_finer_fit" + str(i) +  ".jpg")
        plt.close()
...
estimate_df.to_csv(outDir + enclosing_dir + "/estimate_df_finer_fit")
all_datasets_freqs.to_csv(outDir + enclosing_dir + "/all_dataset_freqs_df_finer_fit")
...
# plt.ylabel("Frequency")
# plt.tight_layout()
# plt.savefig(outDir + enclosing_dir + "/neherResults_binsTogether_finer_fit" + str(0) +  ".jpg")
# plt.close()
...
plt.tight_layout()
plt.savefig(outDir + enclosing_dir + "/comparedEstimates_stripplot_finer_fit.jpg")
plt.close()
