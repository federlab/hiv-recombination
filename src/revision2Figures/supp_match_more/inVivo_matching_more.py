...
GENOME_LENGTH = 700
NE_LIST = ['1000', '5000', '10000', '50000', '1e+05', '5e+05']
NE_LIST_FANCY = [r"$N_e=5\times10^3$",
                r"$N_e=10^4$", 
                r"$N_e=5\times10^4$",
                r"$N_e=10^5$",
                r"$N_e=5\times10^5$"]
...
sim_dataDir1 = "/Volumes/feder-vol1/project/hiv_recombination/data/simulated/2023_08_28_selection_vNE/"
sim_dataDir2 = "/Volumes/feder-vol1/project/hiv_recombination/data/simulated/2023_11_07_selection_vNE/"
inv_dataFile = "/Volumes/feder-vol1/home/evromero/2021_hiv-rec/data/zanini/elife-11282-fig5-data1-v2/divdiv_AB.tsv" 
outDir = "/Volumes/feder-vol1/home/evromero/2021_hiv-rec/results/revision2/supp_figs/supp_match_more/"

dataDirList = [sim_dataDir1, sim_dataDir2]
...
for pipeline_run in dataDirList:
    for curr_dir in os.listdir(pipeline_run):
        if not os.path.isdir(pipeline_run+curr_dir) or curr_dir[0] == '.':
            continue
        ...
        curr_file = pipeline_run + curr_dir + "/slim_output.txt"
...
sns.lineplot(data = ave_dist, x = 'years', y = 'mean_pairwise_dist',
              palette = sns.color_palette("YlGnBu", n_colors=len(NE_LIST_FANCY)),
              hue = 'Ne', style = 'Sim_Rho', ax = ax[1], 
              style_order = [r"$10^{-3}$", r"$10^{-4}$", 
                             r"$2\times10^{-4}$", r"$10^{-5}$",
                             r"$2\times10^{-5}$", r"$2\times10^{-6}$"],
             hue_order = NE_LIST_FANCY)
...
new_labels[0] = r'$N_e$'
new_labels[6] = r'Simulated $\rho$'

ax[1].legend(loc='upper center', bbox_to_anchor=(-0.4, 1.15), ncol=7,
