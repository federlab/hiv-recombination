...
def twin_lineplot(**kwargs):
    ax = plt.twinx()
    sns.lineplot(**kwargs, ax=ax)

g = sns.FacetGrid(all_stat_dfs, col='Sim_float_rho', col_wrap=3, height=5, aspect=1.5)
g.map(sns.histplot, 'Dist_X_Time', kde = True,  stat = 'density', bins = 300)
g.map(twin_lineplot, data=all_rho_bins, x='Dist_X_Time', y='D_Ratio', hue='Sim_float_rho',
             linewidth = 3, palette=sns.color_palette("coolwarm", n_colors=len(all_rho_bins['Sim_float_rho'].unique())))

# print(all_stat_dfs)
# myfacets = sns.FacetGrid(all_stat_dfs, col='Sim_float_rho', col_wrap=3, height=5, aspect=1.5)
# myfacets.map()
# myfacets.map(sns.lineplot, data=all_rho_bins, x='Dist_X_Time', y='D_Ratio', hue='Sim_float_rho',
#              linewidth = 3, palette=sns.color_palette("coolwarm", n_colors=len(all_rho_bins['Sim_float_rho'].unique())))
