...
#We will add some bootstrapping to our fits to see if we can get a measure of 
#confidence on it. 
...
outDir = '/net/feder/vol1/home/evromero/2021_hiv-rec/results/zanini/paper_filtering/fits/bootstrapped_rhos/'
...
for currFile in dataFiles:
    ...
    for date in unique_dates:
        ...
        if n_current > 20:
            ...
            windows_with_points = []
            #check which windows have values in them
            for window in windowStarts:
                winStart = window
                winEnd = window + WINSIZE
                ...
                if not curr_window.empty:
                    windows_with_points.append(window)
            ...
            for i in range(NUM_BOOTSTRAPS):
                ...
                for window in windows_with_points:
                    curr_points = curr_window[curr_window['dist'].between(winStart, winEnd)]
                    # print(len(curr_points.index), file = sys.stderr)
                    #sample 20 points with replacement in this window
                    curr_sample = curr_points.sample(n = 5, replace = True)
                    sampled_data.append(curr_sample)
                sampled_data = pd.concat(sampled_data)

                #fit estimates to our sampled data
                #start by setting parameters for our fit
                params = Parameters()
                params.add('rho', value=0.001)
                params.add('n', value=len(sampled_data.index), vary=False)

                out = minimize(residual, params, args=(timepoint_df['dist'],), kws={'data': timepoint_df['r_squared']})
                
                # print(sampled_data, file = sys.stderr)

                



            #start by printing the number of points in each window
            #now we can use a window step of 1
