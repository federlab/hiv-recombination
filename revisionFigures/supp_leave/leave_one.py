...
excl_par_list = ['p1', 'p2', 'p3', 'p4', 'p5', 'p6', 'p7', 'p8', \
                 'p9', 'p11', 'p4_p7']
...
outDir = "/Volumes/feder-vol1/home/evromero/2021_hiv-rec/results/revision/supp_figs/supp_leave/"
...
# outDir = "/net/feder/vol1/home/evromero/2021_hiv-rec/results/revision/supp_figs/supp_leave/"
...
for curr_par in excl_par_list:
    print(curr_par)
    #Get the dataframe for everyone except the current participant
    if curr_par != 'p4_p7':
        excl_par_df = stat_df[stat_df['Participant'] != curr_par]
    else:
        excl_par_df = stat_df[stat_df['Participant'] != 'p4']
        excl_par_df = excl_par_df[excl_par_df['Participant'] != 'p7']
...
for i in range(len(excl_par_list)):
    curr_excl = excl_par_list[i]
    ...
    curr_excl = curr_excl.split('_')
    if len(curr_excl) == 1:
        curr_ax.set_title('Excluding Participant ' +\
                           str(curr_excl[0][1:]), pad = -14)
    else:
        curr_ax.set_title('Excluding Participants ' +\
                           str(curr_excl[0][1:]) + ' and ' +\
                              str(curr_excl[1][1:]), pad = -14)
    curr_ax.ticklabel_format(style = 'sci', axis = 'y', scilimits = (0,0))
    curr_ax.xaxis.set_tick_params(labelbottom=True)
    # curr_ax.set_xticks(curr_ax.get_xticks(), curr_ax.get_xticklabels(), rotation=30, ha='right')
