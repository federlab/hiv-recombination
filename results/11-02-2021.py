...
# outDir = '/Volumes/feder_vol1/home/evromero/2021_hiv-rec/results/zanini/neher_analysis/fits/'
...
def residual(params, dDeltaT, data, rec_error):
    ...
    print(rec_error, file = sys.stderr)
    resids = data - model
...
CUTOFF = 0.03
SUCCESS = 0.01
RUNNAME = str(CUTOFF) + '_' + str(SUCCESS) +  "SmallerBins_fixedC0"
...
params = Parameters()
params.add('c0', min = 0, max = 1, value = 0.15, vary = False)
params.add('c1', min = 0, max = 5, value = 0.26)
params.add('c2', min = 0, max = 5, value = 0.00005)

print(np.unique(all_frequencies_patients['Recomb Error']), file = sys.stderr)
out = minimize(residual, params, args=(all_frequencies_patients['window'],), kws={'data': all_frequencies_patients['recomb_frequencies'],
                                                                                    'rec_error' : all_frequencies_patients['Recomb Error']})

c0_estimate = out.params['c0']
c1_estimate = out.params['c1']
c2_estimate = out.params['c2']
x_vals = list(range(0, max(all_frequencies_patients['window'])))
...
fit_df = pd.DataFrame(list(zip(x_vals, fitted_vals, fitted_vals_paper)), columns= ['x_vals', 'fitted_vals', 'fitted_vals_paper' ])
...
sns.lineplot(x = 'x_vals', y = 'fitted_vals', data = fit_df, color = 'black', label = 'Our Fit')
sns.lineplot(x = 'x_vals', y = 'fitted_vals_paper', data = fit_df, color = 'green', label = 'Neher Fit')
